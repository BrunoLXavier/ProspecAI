...
interface Props { isOpen: boolean; onClose: () => void; resource: any | null }
...
export default function InfrastructureModal({ isOpen, onClose, resource }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: any) => {
      if (resource?.id) return apiClient.patch(`/api/v1/infrastructures/${resource.id}`, data);
      return apiClient.post('/api/v1/infrastructures', data);
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['infrastructure'] }); onClose(); }
  });
          ...
          <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
            <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white dark:bg-slate-800 shadow-xl transition-all">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-start justify-between">
                <div>
                  <Dialog.Title className="text-lg font-semibold text-gray-900 dark:text-white">{resource ? resource.name : t('newResource')}</Dialog.Title>
                  {resource && <p className="text-sm text-gray-500">ID: {resource.id}</p>}
                </div>
