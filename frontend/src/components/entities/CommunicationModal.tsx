...
interface Props { isOpen: boolean; onClose: () => void; comm: any | null }

export default function CommunicationModal({ isOpen, onClose, comm }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: any) => {
      if (comm?.id) return apiClient.patch(`/api/v1/communications/${comm.id}`, data);
      return apiClient.post('/api/v1/communications', data);
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['communications'] }); onClose(); }
  });
      ...
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <div className="fixed inset-0 overflow-y-auto"><div className="flex min-h-full items-center justify-center p-4">
          <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
            <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white dark:bg-slate-800 shadow-xl transition-all">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-start justify-between">
                <div>
                  <Dialog.Title className="text-lg font-semibold text-gray-900 dark:text-white">{comm ? comm.title : t('new')}</Dialog.Title>
                  {comm && <p className="text-sm text-gray-500">ID: {comm.id}</p>}
                </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                <div>
                  ...
                  <input {...register('title', { required: true })} className="w-full px-4 py-2 border rounded-lg" />
                </div>
                <div>
