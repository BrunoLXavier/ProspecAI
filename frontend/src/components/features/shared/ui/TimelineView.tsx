    ...
    <div className={`relative ${className}`}>
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            {/* Dot/Icon container with connectors */}
            <div className="relative flex flex-col items-center">
              {/* Top connector */}
              {showConnectors && !isFirst && (
                <div
                  className={connectorVariants({ status, dashed: items[index - 1]?.dashedConnector })}
                  style={{
                    top: `-${dotSizeOffset[size]}px`,
                    height: `${dotSizeOffset[size]}px`,
                  }}
                />
              )}
              ...
              {showConnectors && !isLast && (
                <div
                  className={`
                    ${connectorVariants({ status: items[index + 1]?.status || 'default', dashed: item.dashedConnector })}
                    flex-1 mt-0 relative
                  `}
                  style={{
                    minHeight: '24px',
                  }}
